...
impl Batch {
    ...
    pub fn commit(mut self) -> crate::Result<()> {
        ...
        {
            // A partial batch may now end the journal. Appending another batch
            // could acknowledge data that recovery would discard with that tail.
            // Poison while holding the writer lock, before publishing any data.
            self.keyspace.is_poisoned.store(true, Ordering::Release);
            log::error!("journal batch write failed; keyspace requires recovery: {error:?}");
            return Err(error);
        }

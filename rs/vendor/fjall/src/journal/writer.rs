...
pub struct Writer {
    ...
    // One-shot test fault after an actual prefix reaches the journal file.
    #[cfg(test)]
    pub(crate) fail_batch_after_items: Option<usize>,
}
...
impl Writer {
    ...
    pub fn write_batch<'a>(
        ...
        for item in items {
            ...
            #[cfg(test)]
            if let Some(remaining) = &mut self.fail_batch_after_items {
                *remaining -= 1;
                if *remaining == 0 {
                    self.fail_batch_after_items = None;
                    // Leave Start + Item(s) on the real journal, without End.
                    // The next write would succeed unless the caller poisons it.
                    self.file.flush()?;
                    return Err(
                        std::io::Error::other("injected mid-batch journal write failure").into(),
                    );
                }
            }
        }

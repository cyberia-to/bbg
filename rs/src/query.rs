...
use crate::proof::{
    QueryProof, prove_axons_in, prove_axons_out, prove_card, prove_coin, prove_file,
    prove_location, prove_neuron, prove_particle, prove_signal, prove_time,
};
...
pub fn bbg_query(state: &BbgState, dim: Dim, key: &[u8; 32]) -> Option<QueryProof> {
    if matches!(dim, Dim::Time | Dim::Signals) && key[8..].iter().any(|&v| v != 0) {
        return None;
    }
    match dim {
...
pub use crate::query_auth::{
    verify_entity, verify_opening_with_context, verify_query, verify_query_at,
};
...
pub fn collect_look_openings(state: &BbgState, trace: &[nox::TraceRow]) -> Vec<LookOpening> {
    ...
    for row in trace {
        ...
        let Some(dim) = Dim::from_u64(ns.as_u64()) else {
            continue;
        };
        let Some(proof) = crate::proof::open_cell(state, dim, key.as_u64() as usize) else {
            continue;
        };
        let value = query_value(&proof);
...
pub fn verify_opening(_lo: &LookOpening) -> bool {
    false
}
...
mod tests {
    ...
    fn ct() -> Goldilocks {
        Goldilocks::ZERO
    } // commitment arg not checked by BbgLookProvider
    ...
    fn look_provider_invalid_namespace_returns_none() {
        ...
        let prov = BbgLookProvider { state: &state };
        assert!(
            prov.look(ct(), Goldilocks::new(99), Goldilocks::ZERO)
                .is_none()
        );
    }
    ...
    fn look_provider_balances_returns_none() {
        ...
        assert!(
            prov.look(
                ct(),
                Goldilocks::new(Dim::Balances as u64),
                Goldilocks::ZERO
            )
            .is_none()
        );
    ...
    fn look_provider_neuron_focus_cell_by_index() {
        ...
        assert_eq!(
            prov.look(ct(), ns, Goldilocks::new(11)),
    ...
    fn proof_look_provider_returns_value_and_accumulates_opening() {
        ...
        assert!(verify_opening_with_context(
            &openings[0],
            &context,
            &state.root(),
            key.as_u64()
    ...
    fn proof_look_provider_accumulates_multiple_openings() {
        ...
        assert_eq!(
            openings.len(),
            2,
            "only successful lookups accumulate openings"
        );
    }
    ...
    fn proof_provider_value_matches_fast_provider_time() {
        ...
        assert_eq!(
            proof.look(ct(), ns, key),
            fast_v,
            "proof and fast providers must agree"
        );
    }
    ...
    fn collect_look_openings_from_real_trace() {
        ...
        assert!(
            ...
            "look must succeed with ProofLookProvider"
        );
        ...
        assert_eq!(
            col_openings.len(),
            1,
            "collect_look_openings should find 1 look row"
        );

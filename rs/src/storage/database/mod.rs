...
impl Database {
    ...
    pub fn coordination_lock(&self, namespace:[u8;32])->StorageResult<Arc<Mutex<()>>> {
        let mut locks=self.shared.coordination.lock().map_err(|_|StorageError::Corrupt("coordination map poisoned"))?;
        locks.retain(|_,lock|lock.strong_count()>0);
        if let Some(lock)=locks.get(&namespace).and_then(std::sync::Weak::upgrade){return Ok(lock);}
        if locks.len()>=64{return Err(StorageError::Limit("live application coordination locks"));}
        let lock=Arc::new(Mutex::new(()));locks.insert(namespace,Arc::downgrade(&lock));Ok(lock)
    }
    ...
    pub(crate) fn check_known_tables(&self) -> StorageResult<()> {
        let state = self.shared.state.lock().map_err(|_| StorageError::Corrupt("panicked database transaction"))?;
...
mod tests {
    ...
    fn coordination_is_shared_bounded_and_reclaims_expired_roles() {
        ...
        drop(held);
        let others:Vec<_> = (1..64u8).map(|n|db.coordination_lock([n;32]).unwrap()).collect();
        assert!(matches!(db.coordination_lock([64;32]),Err(StorageError::Limit(_))));
        drop(others);
        let next = db.coordination_lock([64;32]).unwrap();
        assert!(!Arc::ptr_eq(&first,&next));
        assert!(Arc::ptr_eq(&first,&db.coordination_lock([0;32]).unwrap()));
        drop(next); drop(first); drop(same); drop(db);

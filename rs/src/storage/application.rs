...
mod validation;
pub use archive::{
    ApplicationArchive, ArchiveSeal, ArchiveSummary, MAX_INSPECTION_BYTES, MAX_INSPECTION_ROWS,
};
pub use transfer::{TransferProgress, TransferSource};
...
#[derive(Debug)]
pub struct NamespaceMigration<'a> {
    pub manifest: Particle,
    pub sources: &'a [(Particle, Head)],
}
impl ApplicationStore {
    ...
    pub fn migration_target(&self, namespace: &Particle) -> Result<Option<MigrationTarget>, Error> {
        self.db
            .read(Table::Migration, &fence_key(namespace), 64)?
            .map(|bytes| {
                if bytes.len() != 64 {
                    return Err(Error::Corrupt);
                }
                Ok(MigrationTarget {
                    namespace: bytes[..32].try_into().map_err(|_| Error::Corrupt)?,
    ...
    pub fn apply_migration(
        &self,
        write: &Write<'_>,
        migration: &NamespaceMigration<'_>,
    ) -> Result<Head, Error> {
        if migration.sources.is_empty()
            || migration.sources.len() > 256
            || migration.sources.windows(2).any(|w| w[0].0 >= w[1].0)
            || migration
                .sources
                .iter()
                .any(|(id, _)| *id == write.namespace)
        {
            return Err(Error::Limit);
        }
        self.apply_with(write, |tx| {
            for (origin, expected) in migration.sources {
                ...
                target.extend(write.namespace);
                target.extend(migration.manifest);
    ...
    fn apply_checked(
        ...
        self.db
            .transaction::<_, Error>(|tx| {
                ...
                if tx
                    .get(Table::Migration, &fence_key(&write.namespace), 64)?
                    .is_some()
                {
                ...
                if tx.get(Table::Migration, b"status", 16)?.as_deref() == Some(b"export-v1")
                    || tx
                        .get(Table::Migration, &transfer::stage_key(&write.namespace), 32)?
                        .is_some()
                {
                    return Err(Error::Fenced);
                }
...
fn fence_key(namespace: &Particle) -> Vec<u8> {
    let mut key = b"neuron-fence\0".to_vec();
    key.extend(namespace);
    key
}
fn immutable_put(

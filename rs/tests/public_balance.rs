...
fn full_u64_balance_requires_exact_root_key_and_both_limbs() {
    ...
    assert_eq!(verify_public_balance(&proof, &root, &owner, &token), Some(u64::MAX));
    assert_eq!(verify_public_balance(&proof, &[0; 32], &owner, &token), None);
    ...
    for legacy in [prove_balances(&state, &owner, &token).unwrap(),
    ...
    assert!(prove_public_balance(&state, &owner, &token).is_none(), "stale cache");
    state.refresh_root();
    assert_eq!(verify_public_balance(&proof, &state.root(), &owner, &token), None);
}

...
properties:
  ...
  - membership proof: Lens.open(A, c_i) → v_i, one opening, O(1)
```
...
spending UTXO u (with commitment c, randomness ρ):
  ...
  2. prove A(c) ≠ 0           (commitment exists — one Lens opening)
  3. prove N(n) ≠ 0           (not yet spent — one Lens opening)
...
witness size:             128 KB (active window)          32 bytes (Lens commitment)
non-membership proof:     O(1) bitmap + O(log N) MMR     O(1) Lens opening
membership proof:         O(log N) MMR                   O(1) Lens opening
...
at 10^9 nullifiers:
  ...
  polynomial: 32-byte commitment + 1 Lens opening (~200 byte proof)
```
...
```
PUBLIC INPUTS:
  A_commit:        F_p⁴               commitment polynomial Lens commitment
  N_commit:        F_p⁴               nullifier polynomial Lens commitment
...
PRIVATE WITNESS:
  ...
  commitment_openings (Lens proofs for A(c_i))
  non_membership_openings (Lens proofs for N(n_i) ≠ 0)
...
CONSTRAINTS:
  input validation (4 inputs):              ~4,000
    ...
    A(c) membership: ~200 per input (Lens opening verification)
    N(n) non-membership: ~200 per input (Lens opening verification)
...
```
new UTXO created:     commitment polynomial A(x) extended — holder's Lens proof still valid
...
the polynomial commitment reveals nothing about individual records. Lens opening proofs are zero-knowledge. the privacy guarantees match or exceed the previous SWBF approach:
...
- commitment isolation: opening A(c) reveals nothing about N(n) — A(x) and N(x) are independent polynomial commitments with separate Lens instances

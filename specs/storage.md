...
`TieredStore::archive()` commits COLD's own pending batch and returns its
commit identity, decoupled from the per-block `commit()` that flushes HOT and
WARM. population — moving a (dimension, key) value from WARM into COLD's
...
periodic sweep) selects keys eligible for archival by the same focus/energy
threshold `demote()` already uses, writes them into COLD via `put`, and calls
...
`last_commit`, independent of WARM/HOT's) and retries the same keys — retrying

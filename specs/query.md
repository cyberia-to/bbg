...
The certificate authenticates public reads; it does not itself prove execution.
A future Zheng state-execution integration must derive its relation from the
actual program and bind each active namespace, key, value and all four root
limbs. Its lookup callback must use a certificate verified against that same
root, and verification must authenticate the execution witness. This integration
is not provided by the current committed Zheng dependency.
...
public tables can use `StateCertificate` directly (limit 2^20 fields), with
application-side computation; this is not an execution proof.

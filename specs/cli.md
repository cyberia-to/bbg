...
- it is **not** the signal lifecycle. it does not validate proofs, order a signal chain, detect equivocation, or route networks — those are [[cybergraph]] and [[sync]]. `bbg apply` is the raw `insert`, the same one cybergraph calls after its gate passes.
...
buildable against today's API: `root`, `height`, `stats`, `get`, `dump`, `apply`, `intend`, `finalize`, `prune`, and `prove` (generate + in-process verify + print value). the enabler is not new serialization — it is **reusing the [[tape]] cyber-dialect codec**. sync's `frames.rs` already encodes/decodes signal and intent frames (`encode_signal_frame` / `decode_signals`, `encode_intent_frame`); the tape signal frame carries the state-application shape (neuron, links, height) the CLI folds via `bbg.insert`, box_moves empty (Release 0). the codec should be canonicalized so `bbg-cli` and `sync` share one implementation — it depends only on [[tape]] + bbg, so the CLI pulls no network stack.
...
- args via `clap` (matches `cyber-sync`'s bin).
...
1. codec home — the cyber-dialect signal/intent tape codec lives in `sync/frames.rs` today, operating on sync's envelope `Signal`. `bbg-cli` needs the same frames but only bbg's state-application shape. proposal: lift the cyber-dialect codec into a shared spot (a `cyber-dialect` module in [[tape]], or a tiny shared crate) that both `sync` and `bbg-cli` import — one frame format, one implementation, no duplication. the CLI depends on [[tape]] + bbg only.
